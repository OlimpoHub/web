...
const BACKEND_BASE = process.env.BACKEND_BASE || process.env.NEXT_PUBLIC_API_BASE || 'http://localhost:8080';
...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  try {
    ...
    const body = ['GET', 'HEAD', 'OPTIONS'].includes(req.method || '') ? undefined : JSON.stringify(req.body ?? {});
    ...
    console.log('Proxy: forwardedHeaders sample=', Object.fromEntries(Object.entries(forwardedHeaders).slice(0,10)));
    ...
    try {
      ...
      console.error('Proxy: initial fetch failed:', err?.message ?? err, 'cause=', err?.cause);
